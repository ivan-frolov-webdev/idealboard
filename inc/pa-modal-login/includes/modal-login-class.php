...
/*-----------------------------------------------------------------------------------*/
	...
	public function paml_add_nav_menu_metabox() {
		add_meta_box('paml_metabox_modal_link', __('PA Modal Link', 'pressapps' ), array( $this, 'paml_callback_metabox_modal_link' ), 'nav-menus', 'side', 'high');
...
/*-----------------------------------------------------------------------------------*/
	...
	public function paml_resources() {
		...
		$login_url = $pamlOptions['login-redirect-url'];
		if ( empty ( $login_url ) ) {
			$login_url = $_SERVER['REQUEST_URI'];
		}

		$theme = $pamlOptions['modal-theme'];
		$labels = $pamlOptions['modal-labels'];
	...
	public function paml_print_style_css() {
		...
		if ( $pamlOptions['bkg-color'] ) {
			echo '.ml-content { background-color: ' . sanitize_text_field( $pamlOptions['bkg-color'] ) . "}\n" ;
			if ( $pamlOptions['bkg-color'] != '#ffffff' ) {
				echo '#modal-login input[type="text"], #modal-login input[type="password"] { border: solid 1px ' . $pamlOptions['bkg-color'] . "; }\n" ;
				echo '#modal-login input[type="text"]:focus, #modal-login input[type="password"]:focus { border: solid 1px ' . $pamlOptions['bkg-color'] . "; }\n" ;
			}
		}
		if ( $pamlOptions['font-color'] ) {
			echo '#modal-login, #modal-login h2 { color: ' . sanitize_text_field( $pamlOptions['font-color'] ) . "}\n" ;
		}
		if ( $pamlOptions['link-color'] ) {
			echo '#additional-settings, #additional-settings a, #additional-settings a:hover { color: ' . sanitize_text_field( $pamlOptions['link-color'] ) . "}\n" ;
		}
		if ( $pamlOptions['btn-color'] ) {
			echo '#modal-login .submit .button { background: ' . sanitize_text_field( $pamlOptions['btn-color'] ) . "}\n" ;
...
/*-----------------------------------------------------------------------------------*/
	...
	public function paml_print_custom_css() {
		...
		echo '<style text="text/css" id="modal-login-custom-css">' . sanitize_text_field( $pamlOptions['custom-css'] ) . "</style>\n";
	}
	...
	public function paml_ajax_login() {
		...
		elseif ( isset( $_REQUEST['register'] ) ) {
			...
			if ( is_wp_error( $user_register ) ) {
                ...
                if(isset($pamlOptions['userdefine_password'])){
				...
				echo json_encode( array(
					'registerd'     => true,
                                        'redirect'      => (isset($pamlOptions['userdefine_password'])?TRUE:FALSE),
...
/*-----------------------------------------------------------------------------------*/
	...
	public function paml_register_new_user( $user_login, $user_email ) {
		...
		if ( $errors->get_error_code() )
			return $errors;
                $user_pass = (isset($pamlOptions['userdefine_password']))?$_REQUEST['password']:wp_generate_password( 12, false );
		...
		update_user_option( $user_id, 'default_password_nag', true, true ); // Set up the Password change nag.
                ...
                $subject        = trim($pamlOptions['reg_email_subject']);
                $body           = trim($pamlOptions['reg_email_template']);
...
/*-----------------------------------------------------------------------------------*/
	...
	public function paml_login_form() {
		global $user_ID, $user_identity, $pamlOptions;
		wp_get_current_user();
...
/*-----------------------------------------------------------------------------------*/
	...
	public function modal_login_btn( $login_text = 'Login', $logout_text = 'Logout', $show_admin = 1 ) {
		...
		global $pamlOptions;
		$logout_url = $pamlOptions['logout-redirect-url'];
